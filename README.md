...
**Anthropic `/v1/messages` ignores truncate-middle**
On LiteLLM `1.57.2`, Anthropic proxy handlers may bypass callback truncation.
This branch includes `scripts/patch-litellm-anthropic-truncate.py`, and
`start-stack.sh` applies it automatically by default.

Disable auto-patch with:
```bash
LITELLM_AUTO_PATCH_ANTHROPIC=0 ./scripts/start-stack.sh
```
